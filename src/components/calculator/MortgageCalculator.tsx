...
/* ── Amortization Line Chart (Bankrate-style) ── */
...
export default function MortgageCalculator({ lang = "en" }: { lang?: Lang }) {
  ...
  // Raw string state for text inputs so the user can type freely
  ...
  const inputCls = "w-full rounded-xl border border-gray-200 bg-white py-3 text-base font-semibold text-gray-900 outline-none transition focus:border-brand-300 focus:ring-4 focus:ring-brand-50";
    ...
    <div className="mx-auto max-w-5xl">
      <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
        ...
        <div className="space-y-5">
          <div>
            <label className="mb-1.5 block text-sm font-semibold text-gray-900">{t.loanAmount}</label>
            <div className="relative">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-semibold text-gray-900">{t.interestRate}</label>
            <div className="relative">
          ...
          <div>
            <div className="mb-1.5 flex items-center gap-1.5">
              <label className="text-sm font-semibold text-gray-900">{t.repaymentRate}</label>
          ...
          <div>
            <div className="mb-1.5 flex items-center gap-1.5">
              <label className="text-sm font-semibold text-gray-900">{t.fixedPeriod}</label>
              <InfoTooltip text={t.infoFixed} />
            </div>
            <select value={fixedRatePeriod} onChange={(e) => setFixedRatePeriod(Number(e.target.value))}
              className={`${inputCls} cursor-pointer appearance-none pl-4 pr-10`}>
              {[5, 10, 15, 20, 25, 30].map((y) => <option key={y} value={y}>{y} {t.years}</option>)}
            </select>
          </div>

          <div>
            <label className="mb-1.5 block text-sm font-semibold text-gray-900">{t.extraPayment}</label>
            <div className="relative">
              <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">€</span>
              <input type="text" inputMode="numeric"
        ...
        <div>
          ...
          {tab === "breakdown" && (
            <div className="space-y-8 pt-6">
              <h2 className="text-xl font-bold text-gray-900">{t.breakdownTitle}</h2>
              ...
              {fixedRatePeriod < payoffYears && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
                  <p className="text-sm font-semibold text-amber-800">{t.remainingBalance} {t.afterFixedPeriod} ({fixedRatePeriod} {t.years})</p>
                  <p className="mt-1 text-2xl font-bold text-amber-900">€{fmt(result.remainingBalance)}</p>
                </div>
              )}
              ...
              <div className="grid items-center gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold tabular-nums text-gray-900">€{fmt(loanAmount)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold tabular-nums text-gray-900">€{fmt(result.totalInterest)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {tab === "amortization" && (
            <div className="space-y-8 pt-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{t.amortizationTitle}</h2>
                <p className="mt-1 text-sm text-gray-500">{t.amortizationDesc}</p>
              </div>

              {/* Date range */}
              <div className="flex items-center gap-4">
                <div className="rounded-lg border border-gray-200 px-4 py-2">
                  <p className="text-xs text-gray-400">{t.firstPayment}</p>
                  <p className="text-base font-bold text-gray-900">{monthNames[startMonth]} {startYear}</p>
                </div>
                <span className="text-lg text-gray-300">→</span>
                <div>
                  <p className="text-xs text-gray-400">{t.lastPayment}</p>
                  <p className="text-base font-bold text-gray-900">{monthNames[lastPaymentMonth]} {lastPaymentYear}</p>
                </div>
                {/* Expand all toggle */}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      <th className="px-4 py-3 text-left font-semibold text-gray-700">{t.dateLabel}</th>
                      <th className="px-4 py-3 text-right font-semibold text-gray-700">{t.principalLabel}</th>
                      <th className="px-4 py-3 text-right font-semibold text-gray-700">{t.interestLabel}</th>
                      {extraAnnualPayment > 0 && (
                        <th className="px-4 py-3 text-right font-semibold text-gray-700">{t.extraLabel}</th>
                      )}
                      <th className="px-4 py-3 text-right font-semibold text-gray-700">{t.balanceLabel}</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <React.Fragment key={yr.year}>
                          <tr onClick={() => toggleYear(yr.year)}
                            className={`cursor-pointer select-none border-b border-gray-100 transition hover:bg-gray-50 ${isFixedEnd ? "bg-amber-50/50" : ""}`}>
                            <td className="px-4 py-3 font-semibold text-brand-600">
                              <span className="mr-1.5 inline-block text-xs">{isExpanded ? "−" : "+"}</span>
                              {startYear + yr.year - 1}
                            </td>
                            <td className="px-4 py-3 text-right tabular-nums">€{fmt(yr.principal)}</td>
                            <td className="px-4 py-3 text-right tabular-nums">€{fmt(yr.interest)}</td>
                            {extraAnnualPayment > 0 && (
                              <td className="px-4 py-3 text-right tabular-nums">€{fmt(yr.extraPayments)}</td>
                            )}
                            <td className="px-4 py-3 text-right font-semibold tabular-nums">€{fmt(yr.endBalance)}</td>
                          </tr>
                          {isExpanded && yr.months.map((m) => (
                            <tr key={m.month} className="border-b border-gray-50 bg-gray-50/50 text-xs text-gray-500">
                              ...
                              <td className="px-4 py-2 text-right tabular-nums">€{fmt(m.principal)}</td>
                              <td className="px-4 py-2 text-right tabular-nums">€{fmt(m.interest)}</td>
                              {extraAnnualPayment > 0 && (
                                <td className="px-4 py-2 text-right tabular-nums">{m.extraPayment > 0 ? `€${fmt(m.extraPayment)}` : "—"}</td>
                              )}
                              <td className="px-4 py-2 text-right font-medium tabular-nums">€{fmt(m.balance)}</td>
                            </tr>
                          ))}
                        </React.Fragment>
                      );
