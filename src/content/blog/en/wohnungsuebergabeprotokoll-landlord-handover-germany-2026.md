...
lang: "en"
tags: ["Wohnungsübergabeprotokoll", "Mietkaution", "handover", "move-in", "move-out", "evidence"]
---
...
- Address of the unit, including floor and apartment number.
- Full names of all parties present (landlord or representative, tenant or representatives, witnesses if any).
- Reference to the lease (Mietvertrag) it relates to, by date or contract number.
- Whether this is an Einzugs- or Auszugsprotokoll.

### Meter readings

The single most disputed line at year-end Nebenkostenabrechnung. Capture:

- Electricity meter (Stromzähler), with meter number and reading.
- Gas meter (Gaszähler) if applicable.
- Water meter, cold and hot separately.
- Heating allocator readings (Heizkostenverteiler) if individual heating cost allocation applies.
- Indicate whether each meter was photographed.
...
- Apartment door keys: how many.
- Building front door, cellar door, attic, mailbox keys.
- Garage or Stellplatz remote.
- Common-area keys (Müllraum, Fahrradkeller).
- Window or balcony keys if applicable.
...
- Floor condition (Parkett, Laminat, Fliesen, Teppich) with type and any defects.
- Wall and ceiling condition, including any holes, marks, discolouration, or evidence of mould.
- Doors, frames, hinges, locks.
- Windows: glazing, frames, seals, operation of handles.
- Fixtures and fittings (light switches, sockets, blinds, radiators).
- For kitchen and bathroom: appliances (with type, brand, age), sanitary fixtures, tiles, silicone joints, taps.
...
- Room, location within the room.
- Type of defect (scratch, hole, stain, missing item).
- Approximate size or quantity.
- Photo reference number from your photo set.
- Whether this is being noted at move-in (pre-existing) or at move-out (claimed against tenant).

### Closing statements

- Signature of both parties with date.
- A line for the tenant or landlord to add observations or disputes before signing.
- A reservation of rights clause for hidden defects: "Weitere verdeckte Mängel bleiben vorbehalten und können innerhalb von 6 Monaten geltend gemacht werden." This is the legal escape clause for defects that could not be detected by reasonable inspection at the time.
...
- If an open defect is **not** listed in the signed protocol, it is presumed to have been accepted by the receiving party. At move-in this means the tenant cannot complain later. At move-out this means the landlord cannot deduct for it.
- If a hidden defect emerges later, the receiving party may still raise it, typically within 6 months of handover, provided they reserved the right to do so in the protocol.
...
For move-in:
- Schedule the handover **before** the tenant signs the actual receipt of keys.
- Block at least 60 minutes for a typical apartment. Rushing leads to missed defects.
- Daylight, if possible. Defects are easier to see by natural light.

For move-out:
- Schedule the appointment **before** the lease end date, ideally 1–3 days before.
- The unit should be empty of the tenant's belongings, broom-clean (besenrein), and any agreed Schönheitsreparaturen completed.
- If the tenant has not vacated by the handover slot, the appointment is invalidated. You can claim continued use of the property (Nutzungsentschädigung) until proper handover.
...
- Defects logged at move-out and not present at move-in: deductible against the deposit, subject to the normal-wear-vs-damage distinction (covered in the [Mietkaution handbook](/blog/mietkaution-handbook-landlords-germany-2026)).
- Defects at move-out that were also at move-in: not deductible, the tenant did not cause them.
- Defects at move-out that look new but are not in the protocol: hard to deduct without supporting photos and a credible explanation for why the landlord could not have seen them at the time.
- Hidden defects emerging within 6 months: deductible if the reservation clause was included and the defect is genuinely hidden.
...
8. Trying to charge for normal wear via a void Schönheitsreparaturen clause. The protocol cannot resurrect an invalid clause. See the [Mietkaution handbook](/blog/mietkaution-handbook-landlords-germany-2026) for the current case law on these clauses.
...
- The Einzugsprotokoll baseline determines what the [Mietkaution](/blog/mietkaution-handbook-landlords-germany-2026) can be used for at move-out.
- Meter readings feed directly into the Nebenkostenabrechnung. Without them, the Verteilerschlüssel calculation for water, heating, and electricity falls apart.
- A clean protocol shortens the time to re-let. You can advertise the next tenancy with confidence about condition and can do the new Einzugsprotokoll the same week.
- The protocol's quality directly affects your effective vacancy rate (less time spent in dispute, fewer days the unit sits unrented). Use the [rental yield calculator](/rental-yield-calculator) to model how a 1–2% vacancy improvement compounds over a 10-year hold.
